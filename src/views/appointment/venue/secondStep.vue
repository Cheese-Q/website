    ...
    <div class="appoit">
        <el-card>
            ...
            <div class="block">
                ...
                <el-button v-loading.fullscreen.lock="fullscreenLoading" type="primary" @click="next()" @blur="check()" class="btn">提交</el-button>
            </div>
        </el-card>
    </div>
    ...
    methods: {
        getMaxcount() {
            ...
            venueApi.getVenueReserveInfo(this.id).then(response=>{
                this.maxPeople = response.data.data.venueInfo.maxPeople;
                venueApi.getPeopleCount(this.id,this.reserveTime).then((response) => {
                  this.peopleCount = response.data.data.peopleCount;
                  this.maxCount = this.maxPeople-this.peopleCount
            });
            })
        },
        check() {},
        next() {
            ...
            if (
                ...
                this.appointTable[len - 1].mobile == null ||
                this.appointTable[len - 1].identity == "" ||
                this.appointTable[len - 1].userName == "" ||
                this.appointTable[len - 1].mobile == ""
            ) {
                this.$message({
                    type: "warning",
                    message: "信息不能为空",
            ...
            venueApi.verifyReserve(this.id, this.reserveTime, this.appointTable[len - 1]).then((response) => {
                if (response.data.code == 200) {
                    venueApi.getReserveCount(this.memberId, this.id, this.reserveTime).then((response) => {
                        ...
                        if (this.count > 4) {
                            ...
                            for (var i = 0; i < len; ++i) {
                                ...
                                this.appointTable[i].venueId = this.id;
                            }

                            venueApi.reserveVenue(this.appointTable).then((response) => {
                                this.$router.push({
                                    path: "/appointment/venue/thirdStep/" + this.id,
                                });
                            });
                        }
                    });
        ...
        pushNewAppoint(index) {
            if (
                ...
                this.appointTable[index].mobile == null ||
                this.appointTable[index].identity == "" ||
                this.appointTable[index].userName == "" ||
                this.appointTable[index].mobile == ""
            ) {
                this.$message({
                    type: "warning",
                    message: "信息不能为空",
                ...
                if(index>0){
                    ...
                    if(this.appointTable[i].identity == this.appointTable[index].identity&&this.appointTable[i].userName == this.appointTable[index].userName) {
                        this.$message({
                        type: "warning",
                        message: "预约人信息不能重复",
                        });
